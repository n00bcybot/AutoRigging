...
locatorsDict = {
    ...
    'spine02_loc': [3.909814713203253, 122.78129034666335, 2.591755210615142],
    'spine03_loc': [-1.5726682459206813, 138.25796503307137, -1.941777543511742],
    'spine04_loc': [-5.300959321115471, 148.43186463177227, -3.0205814470408088]
}
...
locatorsDict_keys=locatorsDict.keys()           # Creating dict list of keys from the dictionary

list_B=[]                                       # Converting it to regular list. This step is necessary, since dict list
for i in locatorsDict_keys:                     # is not iterable
    list_B.append(i)

dictNew={}                                      # Declaring the new list
for i in armLocs:                               # For each item in list_A, if the item is in list_B
    if i in list_B:                             # add it to the dictNew
        dictNew[i]=locatorsDict[i]              # dictNew[item] becomes the key, = , dictOld[i] gets the corresponding
                                                # values
