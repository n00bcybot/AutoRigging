...
class GuiWindow:
    amount = {'fruit1': '10', 'fruit2': '20','fruit3': '30','fruit4': '40'}

    def __init__(self, window, title, size):

        self.locatorsTemplate = {
        ...
        self.thumbLocators = ['l_thumb01_loc', 'l_thumb02_loc', 'l_thumbNub_loc']
        self.indexLocators = ['l_indexFinger01_loc', 'l_indexFinger02_loc', 'l_indexFinger03_loc', 'l_indexFingerNub_loc']
        self.middleLocators = ['l_middleFinger01_loc', 'l_middleFinger02_loc', 'l_middleFinger03_loc', 'l_middleFingerNub_loc']
        self.ringLocators = ['l_ringFinger01_loc', 'l_ringFinger02_loc', 'l_ringFinger03_loc', 'l_ringFingerNub_loc']
        self.pinkyLocators = ['l_pinkyFinger01_loc', 'l_pinkyFinger02_loc', 'l_pinkyFinger03_loc', 'l_pinkyFingerNub_loc']
        self.armLocators = ['l_clavicle_loc', 'l_clavicleNub_loc', 'l_upperArm_loc', 'l_foreArm_loc', 'l_hand_loc']
        self.spineLocators = ['root_loc', 'COMOffset_loc', 'COM_loc', 'pelvis_loc', 'spine01_loc', 'spine02_loc', 'spine03_loc',
                     'spine04_loc', 'neck01_loc', 'neck02_loc', 'head01_loc', 'head02_loc', 'headNub_loc']
        self.legLocators = ['l_thigh_loc', 'l_calf_loc', 'l_heel_loc', 'l_toe_loc', 'l_toeNub_loc']
        self.eyeLocators = ['l_eye_loc', 'l_eyeNub_loc']
        self.jawLocators = ['jaw_loc', 'jawNub_loc']

        self.handLocators = [self.thumbLocators, self.indexLocators, self.middleLocators, self.ringLocators, self.pinkyLocators]
        self.allLists = [self.armLocators, self.legLocators, self.spineLocators, self.eyeLocators, self.jawLocators, self.thumbLocators,
                         self.indexLocators, self.middleLocators, self.ringLocators,self.pinkyLocators]
        self.allChains = [self.armLocators, self.legLocators, self.spineLocators, self.eyeLocators, self.jawLocators, self.handLocators, self.allLists]


    def helpFunction(self):

        print(GuiWindow.amount)

    def selectedList(self):

        selected = cmds.optionMenuGrp(optMenu, query=True, sl=True) - 1
        return self.allChains[int(selected)]

    def windowFunction(self):
        if cmds.window(newWindow.window, query=True, exists=True):
            cmds.deleteUI(newWindow.window)



        cmds.window(newWindow.window, title=newWindow.title, widthHeight=newWindow.size)
        ...
        cmds.button(label='Spawn Locators', command=GuiWindow.helpFunction, height=30)
        cmds.separator(height=2, st='none')
        cmds.button(label='Create Dictionary', command=GuiWindow.helpFunction, height=30)
        cmds.separator(height=2, st='none')
        cmds.button(label='Spawn Joints', command=GuiWindow.helpFunction, height=30)

        cmds.showWindow()


newWindow = GuiWindow(window=True, title='Rigging Tools', size=(400, 400))
newWindow.windowFunction()
